...
## 다음 배포에 함께 나갈 것 (코드는 이미 들어감)

- **화면보호기 기본 대기시간 5분 → 10분** (2026-09-02, 사용자 요청)
  `DEFAULT_SETTINGS.screensaverMinutes` 를 10 으로. 함께 고친 것: 옛 파일용 이관 코드가
  "값이 없으면 5" 로 덮어쓰는데 **처음 설치도 '값이 없는' 상태**라 그 길로 빠졌다 —
  기본값을 바꿔도 새 사용자에게 닿지 않을 뻔했다. 이관은 firstRun 이 아닐 때만 돌게 했다.
  이미 쓰던 사람의 값은 그대로다(잠금 0 → 5분, 잠금 1 이상 → 0, 직접 정한 값 → 유지).
  단독 배포하지 않고 **다음 기능 배포에 얹는다**(사용자 지시).
