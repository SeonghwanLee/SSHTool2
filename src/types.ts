...
export interface SessionInfo {
  ...
  /**
   * true 면 접속을 막는다(0.77.0). 목록에는 흐리게 남는다 — 지우면 설정·비밀번호까지
   * 사라지지만, 점검 중이거나 잠시 쓰지 않는 서버는 **그대로 두고 실수만 막고** 싶다.
   * 터미널·SFTP·원격 데스크톱 접속과 자동 재접속이 모두 멈춘다. 이미 열려 있는 탭은
   * 건드리지 않는다 — 쓰고 있는 연결을 뒤에서 끊는 편이 더 위험하다.
   */
  disabled?: boolean;
}
